...
if (isset($_POST['reset_pwd'])) {
  ...
  if (mysqli_num_rows($checkEmail) > 0) {
    ...
    $query = "INSERT INTO reset_pw (reset_email, reset_code, reset_token, reset_status) VALUES (?,?,?,?)";
    $reset = $mysqli->prepare($query);
    $rc = $reset->bind_param('ssss', $reset_email, $reset_code, $reset_token, $reset_status);
    $reset->execute();
    if ($reset) {
      $success = "Instruksi Pengaturan Ulang Kata Sandi Dikirim Ke Email Anda";
      // && header("refresh:1; url=index.php");
    } else {
      $err = "Silakan Coba Lagi Atau Coba Nanti";
    }
  } else {
    $err = "Tidak ada akun dengan email tersebut";
  }
}
