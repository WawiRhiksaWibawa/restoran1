...
if (isset($_POST['ChangeProfile'])) {
    ...
    $email = $_POST['email'];
    $Qry = "UPDATE users SET nama_user =?, email =? WHERE id_users =?";
    $postStmt = $mysqli->prepare($Qry);
    //bind paramaters
    $rc = $postStmt->bind_param('sss', $nama_user, $email, $id_users);
    $postStmt->execute();
    //declare a varible which will be passed to alert function
    if ($postStmt) {
        $success = "Akun Diperbarui" && header("refresh:1; url=dasbor_kasir.php");
    } else {
    ...
    if (!$error) {
        ...
        if (mysqli_num_rows($res) > 0) {
            ...
            if ($old_password != $row['password']) {
                ...
                //bind paramaters
                $rc = $stmt->bind_param('si', $new_password, $id_users);
                $stmt->execute();

                //declare a varible which will be passed to alert function
                if ($stmt) {
                    $success = "Password Terubah" && header("refresh:1; url=dasbor_kasir.php");
                } else {
                    $err = "Silakan Coba Lagi Atau Coba Nanti";
                }
            }
                            ...
                            <div class="card-body pt-0 pt-md-4">
                                ...
                                <div class="text-center">
                                    ...
                                    <div class="h5 font-weight-300">
                                        <i class="ni location_pin mr-2"></i><?php echo $users->email; ?>
                                    </div>
                                </div>
                            </div>
                        </div>
